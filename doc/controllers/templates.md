...
try
{
    await templatesController.FavTemplatesAsync(userId);
}
...
try
{
    AllTemplates result = await templatesController.AllTemplatesAsync(userId);
}
catch (ApiException e)
{
    // TODO: Handle exception here
...
try
{
    await templatesController.TemplateDetailAsync(
        userId,
        templateId
    );
}
...
| `temperature` | `double` | Query, Required | - |
| `userId` | `int` | Query, Required | - |

## Response Type

[`Task<Models.ProcessTemplate>`](../../doc/models/process-template.md)
...
try
{
    ProcessTemplate result = await templatesController.ProcessTemplateAsync(
        contentId,
        maxResults,
        maxWords,
        temperature,
...
[`Task<Models.GenerateTemplate>`](../../doc/models/generate-template.md)

## Example Usage

```csharp
int userId = 40;
string templateCode = "EEKZF";
string language = "en-US";
int workbookId = 1;
int folderId = 1;
int maxResults = 1;
int words = 100;
string title = "Tsmokeshope";
string description = "tell me about code";
double creativity = 0.5;
try
{
    GenerateTemplate result = await templatesController.GenerateTemplateAsync(
        userId,
        templateCode,
        language,
        workbookId,
...
try
{
    await templatesController.TemplateGroupsAsync();
}
catch (ApiException e)
{
    // TODO: Handle exception here
